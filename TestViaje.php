...
//---------------------------------------------------------------------
public function agregarViaje($destinoV ,$cantMax, $idEmpresa, $idResponsable, $importeV){
    $this->responsable1->Buscar($idResponsable);
    $this->empresa1->Buscar($idEmpresa);
    $this->viaje1->cargar($destinoV ,$cantMax, $this->empresa1, $this->responsable1, $importeV);
    return $this->viaje1->insertar();
}

public function modificarViaje($idViaje, $destinoV ,$cantMax, $idEmpresa, $idResponsable, $importeV){
    $this->responsable1->Buscar($idResponsable);
    $this->empresa1->Buscar($idEmpresa);
...
public function ArrayPasajeros($idViaje){
    $this->viaje1->Buscar($idViaje);
    return $this->viaje1->imprimirPasajeros($this->viaje1->getIdViaje());
}
        ...
        do {           
            ...
            switch ($opcion) {
                case '1':
                    ...
                    echo "ingresar numero de documento: ";
                    $documentoR =trim(fgets(STDIN));
                    echo "ingresar numero de licencia: ";
                    $numeroLicencia =trim(fgets(STDIN));
                    if ($test1->responsable1->AgregarResponsable($nombreR, $apellidoR,$documentoR, $numeroLicencia)) {
